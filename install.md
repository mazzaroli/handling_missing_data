# handling_missing_data guide installation
...
activate handling_missing_data
```
...
To move beyond notebook prototyping, all reusable code should go into the `handling_missing_data/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `handling_missing_data` folder and use the modules inside your notebooks :
...
```python
from handling_missing_data.utils.paths import data_dir
data_dir()
```
